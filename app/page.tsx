  ...
  return (
    <div className="min-h-screen bg-white px-5 pb-8 pt-6">
      ...
      <section className="mb-8 overflow-hidden rounded-[24px] bg-gradient-to-br from-teal-600 to-teal-700 p-5 shadow-xl shadow-teal-900/20">
        <h2 className="text-[30px] font-black leading-tight text-white">
          Predict Your Colleges
        </h2>
        <p className="mt-4 text-[20px] font-medium leading-8 text-white/95">
          Get chance based college predictions in seconds.
        </p>

        <Link
          href="#predictor"
          className="mt-7 flex h-[64px] items-center justify-center rounded-2xl bg-white text-[21px] font-black text-teal-700"
        >
        ...
        <div className="grid grid-cols-4 gap-4">
          {quickActions.map((item) => (
            <Link key={item.label} href={item.href} className="text-center">
              <div className={`mx-auto mb-3 flex h-[72px] w-[72px] items-center justify-center rounded-[24px] ${item.bg} text-4xl`}>
                {item.icon}
              </div>
        ...
        <div className="grid grid-cols-4 gap-4">
          {cities.map((city) => (
            <Link key={city.name} href={`/colleges?city=${city.name}`} className="text-center">
              <img src={city.img} alt={city.name} className="mb-3 h-[72px] w-full rounded-[22px] object-cover" />
      ...
      <section id="predictor" className="scroll-mt-6">
        ...
        <PredictorForm onPredicted={() => setShowResults(true)} />
      </section>
    </div>
  );
}